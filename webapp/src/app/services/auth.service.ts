...
export class AuthService {
  ...
  login(email: string, password: string) {
    return this.http.post<any>(environment.login_url, {email, password}, environment.base_http_headers).pipe(
        map(authResult => {
          ...
          const user = new User(email, authResult.accessToken,
              email[0] === 'd' ? Role.Professor : email[0] === 'a' ? Role.Admin : Role.Student);
  ...
  register(email: string, name: string, surname: string, id: string, password: string, pic: object) {
    return this.http.post(environment.register_url, {
      email, password, name, surname
    }, environment.base_http_headers).pipe(
