...
export class NewVmComponent implements OnInit {
  ...
  onSubmit() {
    if (this.form.invalid)
      return;
    let vm = new VM(0, this.form.get('name').value, this.form.get('path').value, new Date().toString(), this.form.get('cpu').value, this.form.get('ram').value, this.form.get('disk').value);
    delete vm.id;
