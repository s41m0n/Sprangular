...
export class TabAdminVmModelContComponent implements OnInit, OnDestroy {
  model: VmModel = null;
  ...
  ngOnInit(): void {
    ...
    this.courseService.currentCourseSubject.asObservable().pipe(takeUntil(this.destroy$)).subscribe(course => {
      this.course = course;
      if (this.course) {
        this.vmService.getModelForCourse(course).pipe(first()).subscribe(model => this.model = model);
      } else {
        this.model = null;
      }
    });
  ...
  public assignModel(model: VmModel): void {
    this.vmService.assignVmModelToCourse(model, this.course).pipe(first()).subscribe(modelReturned => {
      if (modelReturned) {
        this.vmService.getModelForCourse(this.course).pipe(first()).subscribe(modelToSet => this.model = modelToSet);
      }
    });
  }
}
